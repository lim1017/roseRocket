context('Actions', () => {
  ...
  it('Create an appointment, check data is saved properly, then delete', () => {
    ...
    cy.get('[data-cy=notes]').find('textarea').should('have.value', 'raining today')
  ...
  it('Create two appointments, Swap drivers, then check Bobs Profile', () => {
    ...
    cy.get('.AppointmentsContainer-container-411').find('div').should('contain', 'Pickup')
    cy.get('.AppointmentsContainer-container-411').find('div').should('contain', 'Dropoff')
    ...
    cy.get('.AppointmentsContainer-container-411').find('div').should('not.contain', 'Pickup')
    cy.get('.AppointmentsContainer-container-411').find('div').should('not.contain', 'Dropoff')
  ...
  it('Create an appointment that results in a conflict, successfully overwrites old appointment', () => {
    ...
    cy.get(':nth-child(3) > .MuiInputBase-root > .MuiInputBase-input').click("center").type("{leftarrow}{leftarrow}{leftarrow}{leftarrow}{leftarrow}{leftarrow}{backspace}8"	)
    cy.contains('Save').click()
